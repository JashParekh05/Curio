...
def _plan_sections(topic_name: str, difficulty: str = "intermediate", path_context: list[str] | None = None) -> list[dict]:
    ...
    def _log_round(n: int, judge: dict, weak: list[int]) -> None:
        if weak:
            issues = {s.get("section_index"): s.get("issue", "")
                      for s in judge.get("sections", []) if not s.get("ok", True)}
            logger.info(f"[section_planner] '{topic_name}' judge round {n}: "
                        f"score={judge.get('overall_score')} weak={weak} issues={issues}")
        else:
            logger.info(f"[section_planner] '{topic_name}' judge round {n}: "
                        f"score={judge.get('overall_score')} all ok")

    # Judge once up front, then judge again after every revision so the plan we
    # return is always confirmed by a judge pass (no unverified trailing revise).
    try:
    ...
    for attempt in range(MAX_REVISIONS):
        ...
        _log_round(attempt + 1, judge, weak)
        if not weak:
            break
        try:
            ...
            logger.warning(f"[section_planner] revision/re-judge failed for '{topic_name}': {exc}")
            break
    else:
        # Exhausted MAX_REVISIONS; log the verdict on the final revised plan.
        _log_round(MAX_REVISIONS + 1, judge, _sections_needing_revision(judge))
